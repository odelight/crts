...
public class Game extends JFrame {
	...
	public Game(Network n, AbstractColor playerColor, InputReader ir) {
		...
		if (ir == null)
			ir = new InputReader(new Player(playerColor), md);
		this.ir = ir;
	...
	private void initMap(int width, int height) {
		int cornerSize = 200;
		{
			List<Point> player1Blob = Arrays.asList(new Point[] {
					new Point(0, 0), new Point(cornerSize, 0),
					new Point(cornerSize, cornerSize),
					new Point(0, cornerSize), new Point(0, 0) });
			CrtsPolygon player1 = new CrtsPolygon(player1Blob, map);
			map.putBlob(AbstractColor.PLAYER1, player1);
		}
		{
			List<Point> player2Blob = Arrays.asList(new Point[] {
	...
	private static void printUsage() {
		System.out
				.println("Usage: 1. Call with zero arguments to start a single player game. "
	...
	private static void handleConstructorIOException(IOException e) {
		System.out
				.println("Network issue - possibly incorrect port and/or hostname?");
		printUsage();
